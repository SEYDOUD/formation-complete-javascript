...
class AmbientMixer{
    ...
    init(){
        try {
            ...
            sounds.forEach((sound) => {
                this.currentSoundState[sound.id]
            })
    ...
    setupEventListeners(){
        ...
        document.addEventListener('click' , async (e) => {
            ...
            // Checkif deletebutton is clicked
    ...
    setSoundVolume(soundId , volume){
        ...
        // Calculate effective volume with master
        const effectiveVolume = (volume * this.masterVolume) / 100

        // Update the sound Voulume with the scaledvolume
        const audio = this.soundManager.audioElements.get(soundId)
    ...
    updateMainPlayButtonState(){
        ...
        // Update the main button and the internal
    ...
    saveCurrentPreset(){
        ...
        if(this.presetManager.presetNameExists(name)){
            alert(`A preset with name ${name} already exists`)
            return;
        }
